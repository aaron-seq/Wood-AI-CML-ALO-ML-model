    ...
    def forecast_batch(
        ...
        for _, row in df.iterrows():
            ...
            if 'last_inspection_date' in row:
                last_inspection = pd.to_datetime(row['last_inspection_date'])
    ...
    def generate_forecast_summary(self, df: pd.DataFrame) -> Dict:
        ...
        summary = {
            ...
            'inspections_needed_next_12_months': len(
                forecast_df[
                    pd.to_datetime(forecast_df['next_inspection_date']) < 
                    datetime.now() + timedelta(days=365)
                ]
            ),
            'top_priority_inspections': forecast_df.nsmallest(10, 'remaining_life_years')[[
                'id_number', 'remaining_life_years', 'next_inspection_date', 'risk_level'
