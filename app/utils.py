...
def validate_cml_dataframe(df: pd.DataFrame) -> dict[str, Any]:
    ...
    try:
        ...
        if null_counts.any():
            ...
            validation_results["warnings"].append(f"Null values found: {null_cols.to_dict()}")
        ...
        invalid_thick = df[
            (df["thickness_mm"] <= MIN_THICKNESS_MM) | (df["thickness_mm"] > MAX_THICKNESS_MM)
        ]
...
def save_sme_override(override_data: dict[str, Any], override_file: Path) -> None:
    ...
    try:
        ...
        if isinstance(override_data.get("override_date"), datetime):
        ...
        raise OSError(f"Cannot write override to {override_file}: {e}") from e
...
def generate_elimination_report(predictions_df: pd.DataFrame) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        "summary": {
            ...
            "elimination_rate": round(len(eliminations) / total_cmls * 100, 1)
            if total_cmls > 0
            else 0,
        }
    }
    ...
    if "commodity" in eliminations.columns and len(eliminations) > 0:
        report["elimination_by_commodity"] = eliminations.groupby("commodity").size().to_dict()

    if "feature_type" in eliminations.columns and len(eliminations) > 0:
        report["elimination_by_feature"] = eliminations.groupby("feature_type").size().to_dict()
    ...
    if "elimination_probability" in eliminations.columns and len(eliminations) > 0:
        ...
        if available_cols:
            report["top_elimination_candidates"] = eliminations.nlargest(
                20, "elimination_probability"
            )[available_cols].to_dict("records")
    ...
    if "elimination_probability" in predictions_df.columns:
        ...
        available_marginal_cols = [col for col in marginal_cols if col in predictions_df.columns]
        ...
        if available_marginal_cols:
            ...
            report["marginal_cases"] = marginal_cases[available_marginal_cols].to_dict("records")
