...
def test_calculate_remaining_life():
    ...
    remaining_life = forecaster.calculate_remaining_life(
        current_thickness=10.0, corrosion_rate=0.10, min_thickness=3.0
    )
...
def test_calculate_remaining_life_zero_corrosion():
    ...
    remaining_life = forecaster.calculate_remaining_life(current_thickness=10.0, corrosion_rate=0.0)
...
def test_calculate_inspection_interval():
    ...
    interval = forecaster.calculate_inspection_interval(
        remaining_life_years=20.0, corrosion_rate=0.10
    )
...
def test_calculate_risk_level():
    ...
    risk = forecaster.calculate_risk_level(
        remaining_life_years=0.5, corrosion_rate=0.30, current_thickness=4.0
    )
    assert risk == "CRITICAL"

    # Low risk
    risk = forecaster.calculate_risk_level(
        remaining_life_years=20.0, corrosion_rate=0.05, current_thickness=12.0
    )
    assert risk == "LOW"
