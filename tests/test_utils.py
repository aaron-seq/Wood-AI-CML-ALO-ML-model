...
from app.utils import calculate_inspection_schedule, validate_cml_dataframe
...
def test_validate_cml_dataframe_missing_columns():
    ...
    df = pd.DataFrame({"id_number": ["CML-001"], "average_corrosion_rate": [0.12]})
...
def test_calculate_inspection_schedule():
    ...
    result = calculate_inspection_schedule(
        corrosion_rate=0.12, thickness=9.5, min_thickness=3.0, safety_factor=1.5
    )
